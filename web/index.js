...
var FileSystem = class {
  ...
  async writeText(path, text2) {
    return await this.write(path, encode(text2));
  }
...
var WebtorrentHub = class {
  ...
  #remoteAddr;
  constructor(ws, infoHash, localAddr, remoteAddr2) {
    if (remoteAddr2) {
...
var networkService_default = {
  ...
  queue: [],
  smokeClient: null,
  setNetwork(ws, infoHash, peerId, remoteAddr2) {
    console.log("setting network");
    this.address = peerId;
    const client = new Network({ hub: new WebtorrentHub(ws, infoHash, peerId, remoteAddr2) });
  ...
  manualRecive() {
    ...
    if (textarea !== null) {
      textarea.addEventListener("paste", async (e) => {
        ...
        if (text2) {
          const message = JSON.parse(text2);
...
var client1 = new Network({ hub: new ManualHub("ws://localhost:5001/hub") });
var client2 = new Network({ hub: new ManualHub("ws://localhost:5001/hub") });
client1.Http.listen({ port: 5e3 }, (request) => new Response("hello webrtc"));
var remoteAddr = await client1.Hub.address();
var text = await client2.Http.fetch(`http://${remoteAddr}:5000`).then((r) => r.text());
console.log({ text });

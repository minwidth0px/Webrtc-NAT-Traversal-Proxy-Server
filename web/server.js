...
var HttpWebSocket = class {
  ...
  async #sendListenerRequestInit(urlObject, requestInit) {
    ...
    const url2 = urlObject.path ?? "/";
    const init = { headers, method, url: url2 };
...
var HttpListener = class {
  ...
  async #onRequest(socket) {
    ...
    if (listenerRequestInit === null)
      return await stream.close();
    const url2 = new URL(`http://${socket.local.hostname}:${socket.local.port}${listenerRequestInit.url}`);
...
function resolveHostnameAndPort(input) {
  const url2 = parse(input);
  return [url2.host ?? "localhost", url2.port === null ? 80 : parseInt(url2.port)];
}
...
async function fetch2(net, endpoint, requestInit = {}) {
  ...
  await sendListenerRequestInit(duplex, url2, requestInit);
...
var WebRtcModule = class {
  ...
  async connect(remoteAddress, port, options) {
    ...
    return timeout(awaiter.promise(), { timeout: 4e3, error: new Error(`Connection to '${remoteAddress}:${port}' timed out`) });
  }
...
var WebtorrentHub = class {
  ...
  #remoteAddr;
  constructor(ws2, infoHash2, localAddr, remoteAddr2) {
    if (remoteAddr2) {
      this.#address = remoteAddr2;
    }
    ...
    if (textarea !== null) {
      textarea.addEventListener("paste", async (e) => {
        ...
        if (text) {
          ...
          networkService_default.ICEParams = message;
...
var network = new Network({ hub: new WebtorrentHub(ws, infoHash, peerId, null) });
var network2 = new Network({ hub: new ManualHub("ws://localhost:5001/hub") });
var requestsDiv = document.getElementById("requests");
var server = network;
server.Http.listen({ port: 5e3 }, async (request) => {
  console.log(request);
...
var remoteAddr = await server.Hub.address();
var serverUrl = `http://${remoteAddr}:5000`;
var urlDiv = document.getElementById("url");
var infoHashDiv = document.getElementById("info-hash");
var fullUrl = `${serverUrl}/?infoHash=${infoHash}`;
if (urlDiv !== null) {
